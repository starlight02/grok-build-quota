...
- **服务端探针**：`POST /v1/responses`（默认模型 `grok-4.6`，旧模型 `grok-4.5` 作过渡兜底）判定可用性，配合 `/settings`、`/user`、`/billing` 识别账号类型与用量；模型只是「敲门砖」，额度判定靠状态码 / 限流头 / billing，与模型能力无关。上游换代时用环境变量 `GBQ_PROBE_MODEL` 覆盖（逗号分隔，按序尝试），探针模型下线会自动降级并给出更新提示，无需改代码
- **动态 CLI 版本**：从 xAI 官方安装通道的 `https://x.ai/cli/stable` 获取当前 Grok Build CLI 版本，写入请求头；失败时使用备用通道和安全降级版本
- **细分状态**：可用 / 额度耗尽 / 限流 / 消费上限 / 鉴权失败 / 访问拒绝 / 已禁用 / Token 过期 / 刷新失败 / 无效文件 / 网络错误
- **动态额度周期**：Free 与付费账号同时兼容 responses rate-limit 和 billing；日 / 周 / 月 / 滚动窗口由上游字段或响应头决定，未返回时显示「窗口」而不猜测
- **Token 刷新**：支持「自动刷新」（仅 401/过期时换新）和「强制刷新」（即使 access token 可用也在检测前换新），也可对异常行手动刷新
