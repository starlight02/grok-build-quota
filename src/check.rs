...
impl CheckResult {
    ...
    #[cfg_attr(not(feature = "ssr"), allow(dead_code))]
    fn with_plan(mut self, plan: AccountPlan) -> Self {
...
mod ssr {
    ...
    pub async fn check_one(
        ...
        let mut probe = match probe_responses(client, &auth, &responses_url).await {
            ...
            Err(err) => {
                return CheckResult::make(
                    ...
                    // 探测失败不算刷新成功：token 可能已轮换，但本轮未验证可用
                    false,
                    updated_content,
                )
                .with_plan(plan);
        ...
        if probe.status_code == Some(401) && !did_refresh && !auth.refresh_token.trim().is_empty() {
            ...
            match try_refresh(client, &mut auth).await {
                Ok(note) => {
                    ...
                    updated_content = serialize_auth(&auth);
                    info = fetch_plan(client, &auth, &base_url, jwt_claim_tier(&auth.access_token))
                        .await;
                    plan = info.plan;
        ...
        let (status, usable, mut detail) = match status_code {
            Some(200) => {
                ...
                _ if plan.is_paid() => (
                    ...
                    Some(
                        probe
                            .detail
                            .clone()
                            .unwrap_or_else(|| "触发限流（HTTP 429），额度未耗尽".into()),
                    ),
                ),
                _ => (
                    AccountStatus::Exhausted,
            ...
            Some(code) => match probe.code {
                ...
                _ => (
                    ...
                    Some(
                        probe
                            .detail
                            .map(|d| format!("HTTP {code}：{d}"))
                            .unwrap_or_else(|| format!("HTTP {code}")),
                    ),
                ),
            },
            None if probe.network_error => (
                AccountStatus::NetworkError,
                false,
                probe
                    .detail
            ...
            None => (
                AccountStatus::Error,
                false,
                probe.detail.or_else(|| Some("探测失败".into())),
            ),
        };
        ...
        let (status, usable) =
            if !usable && jwt_expired(&auth.access_token) && status_code != Some(200) {
        ...
        let mut result = CheckResult::make(
            account,
            filename,
            status,
            quota,
    ...
    fn parse_billing_payload(data: &Value) -> ParsedBilling {
        ...
        if let Some(arr) = cfg.get("productUsage").and_then(|v| v.as_array()) {
            for item in arr {
                ...
                match name.as_str() {
                    "grokbuild" | "build" => out.build = pct,
                    "api" | "xaiapi" => out.api = pct,
                    "grokchat" | "chat" => out.chat = pct,
                    _ => {}
                }
            }
        }
        out.monthly_limit = cfg
            .get("monthlyLimit")
            .or_else(|| cfg.get("monthly_limit"))
            .and_then(billing_val);
        out.monthly_used = cfg.get("used").and_then(billing_val);
        out
    }
    ...
    async fn fetch_plan(
        ...
        if let Ok(resp) = client
            .get(format!("{base}/user?include=subscription"))
            .headers(headers)
            .send()
            .await
        {
            if resp.status().as_u16() == 200 {
                if let Ok(v) = resp.json::<Value>().await {
                    if let Some(s) = v.get("subscriptionTier").and_then(|x| x.as_str()) {
                        let t = s.trim();
                        if !t.is_empty() {
                            subscription_tiers = Some(t.to_string());
                        }
                    }
                }
            }
    ...
    async fn fetch_billing(
        ...
        // 1) 周额度：GET /v1/billing?format=credits → creditUsagePercent + productUsage
        if let Ok(resp) = client
            ...
            .await
        {
            if resp.status().as_u16() == 200 {
                if let Ok(v) = resp.json::<Value>().await {
                    let p = parse_billing_payload(&v);
                    out.credit_usage_percent = p.credit;
                    out.build_usage_percent = p.build;
                    out.api_usage_percent = p.api;
                    out.chat_usage_percent = p.chat;
                    out.monthly_limit_cents = p.monthly_limit.or(out.monthly_limit_cents);
                    out.monthly_used_cents = p.monthly_used.or(out.monthly_used_cents);
                    out.has_product_usage = p.has_products;
                }
            }
        }

        // 2) 月度：GET /v1/billing → monthlyLimit/used（USD 美分，套餐 SKU 判别）
        if let Ok(resp) = client
            .get(format!("{base}/billing"))
            .headers(headers)
            .send()
            .await
        {
    ...
    fn resolve_auth_record(file: &AuthUpload) -> Result<AuthRecord, String> {
        ...
        let client_id =
            first_string(obj, &["client_id"]).unwrap_or_else(|| DEFAULT_CLIENT_ID.into());
        let token_endpoint =
    ...
    mod tests {
        ...
        fn detects_spending_limit_exhausted() {
            ...
            let body2 = r#"{"error":{"code":"permission-denied","message":"monthly spending limit reached"}}"#;
        ...
        fn weekly_percent_prefers_credit_total() {
            ...
            assert_eq!(p.credit, Some(100.0));
            assert_eq!(p.api, Some(96.0));
            assert_eq!(p.build, Some(2.0));
            assert_eq!(p.chat, Some(2.0));
            assert!(p.has_products);

            let usage = BillingUsage {
                credit_usage_percent: p.credit,
                build_usage_percent: p.build,
                api_usage_percent: p.api,
                chat_usage_percent: p.chat,
                has_product_usage: p.has_products,
                ..Default::default()
            };
